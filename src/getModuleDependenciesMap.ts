...
import { CodemonPluginOptions } from './codemonPlugin.js';
...
export async function getModuleDependenciesMap(
    moduleRelativePath: string,
    moduleFullPath: string,
): Promise<Record<'imports' | 'exports', Record<string, DependencyInfo>>> {
    const sourceCode = await fs.readFile(moduleFullPath, 'utf-8');
    ...
    babel.transformFromAstSync(ast, sourceCode, {
        ...
        configFile: false,
        plugins: [
            function ({ types: t }: CodemonPluginOptions): PluginObj {
                return {
                    visitor: {
                        // Static imports
                        ImportDeclaration(path) {
                            ...
                            path.node.specifiers.forEach(specifier => {
                                ...
                                } else if (t.isImportSpecifier(specifier) && specifier.importKind !== 'type') {
                        ...
                        CallExpression(path) {
                            if (t.isImport(path.node.callee)) {
                                ...
                                if (t.isVariableDeclarator(parentPath.node)) {
                                    const variableName = parentPath.node.id.properties[0].key.name; // assuming destructuring assignment
                        ...
                        ExportNamedDeclaration(path) {
                            if (path.node.declaration) {
                                ...
                                if (t.isFunctionDeclaration(declaration) || t.isVariableDeclaration(declaration)) {
                                    ...
                                    exportsMap[name] = { path: moduleRelativePath, isDefault: false, type: 'samefile' };
                                }
                        ...
                        ExportDefaultDeclaration(path) {
                            if (
                                ...
                                exportsMap[name] = { path: moduleRelativePath, isDefault: true, type: 'samefile' };
                            }
                        },
                    },
                };
