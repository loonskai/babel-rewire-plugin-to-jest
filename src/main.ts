...
const argv = await yargs(hideBin(process.argv))
  .option("webpackConfig", {
    alias: "w",
    describe:
      "Path to the Webpack configuration file (used for module resolution)",
    type: "string",
    demandOption: true,
  })
  .option("files", {
...
async function processFiles(filesPath: string) {
  try {
    ...
    const filesContent = fs.readFileSync(absolutePath, "utf-8");
    ...
    for (const testFilePath of paths) {
      ...
      if (trimmedTestFilePath) {
        ...
        transformFile(trimmedTestFilePath, webpackResolver);
      }
    }

    console.log("Finished Codemod run");
  } catch (error) {
